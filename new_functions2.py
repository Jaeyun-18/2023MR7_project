...
def limit_position(max_position, min_position, position):
    position = min(max_position, position)
    position = max(max_position, position)
    return position
...
def trans_angle(goal_angle):
    ...
    goal_angle = (goal_angle / 360) * 4096 + 2048
    goal_angle = np.vstack([
    goal_angle,
        [[4096-1*goal_angle[1][0]]],  # 8번
        [[4096-1*goal_angle[5][0]]]   # 9번
    ])

    """
    goal_angle[0][0] = limit_position()
    """
